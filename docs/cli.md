...
| `room create <房间ID> [phira_api_endpoint|default] [hidden]` | 创建无人持久空房间，首个普通玩家加入后自动成为房主 |
| `room keep <房间ID> <true|false>` | 设置房间最后一名玩家离开后是否保留 |
...
覆盖后，服务端能够确定房间上下文的 Phira API 查询都会使用房间 endpoint，例如选谱时查 `/chart/<id>`、提交成绩时校验 `/record/<id>`、终端/欢迎语/Web API 展示谱面名与用户名时的服务端解析。用户登录认证 `/me` 保持使用全局 `server_config.yml` 的 `phira_api_endpoint`；MP 服务器也不会尝试改写客户端本机 Phira API 请求行为。

...
这些是 advanced 级别命令。默认推荐使用 `simulation` 进行压力测试。Simulation 不需要 Phira token，在隔离 shadow world 中运行。Real benchmark 需要 Phira token，token 可在 `server_config.yml` 的 `benchmark_phira_tokens` 中配置。

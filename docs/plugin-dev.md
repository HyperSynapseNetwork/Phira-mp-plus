...
> 注意：`phira-config` 已有基于 `config.json` 的 WIT host 基础实现，但 CLI 管理命令、schema 校验、具体变更 key 追踪和写接口 capability enforcement 仍未完成。配置规范与限制见 [plugin-config.md](plugin-config.md)。
...
> 注意：当前 host 仅实现 `simulation.status` 查询，其余 simulation 控制函数仍是占位实现。

...
| **接口数量** | `12` |
| **Host traits** | `WitPluginHost` 已覆盖 12 个接口；部分控制类函数仍返回占位错误 |
...
5. 所有新插件应面向 `abi-wit-v2` 和 `phira-plugin-v2` world 编译。
...
3. 将 `wit_host.rs` 中的 `not yet implemented` 分成两类：真实实现或明确 capability error。
4. 同步 `phira-plugin-sdk` 示例，避免继续把 JSON ABI 描述为当前路径。

...
当前状态: MIGRATION_PHASE 3。WIT lifecycle 完整接线，53 个 host API 方法全部实现（含明确 capability error）。WASM 集成测试已添加（lifecycle + host API + SSE 注册）。WitHostContext 解耦完成。capability 映射合同测试已添加。

剩余收敛工作:

1. capability enforcement 集成测试（WIT room/admin/config/simulation 写操作的授权验证）。
2. SSE 流全链路集成测试（注册→连接→事件翻译）。
3. 同步 `phira-plugin-sdk` 示例，使 WIT/component model 成为唯一当前 ABI 路径。

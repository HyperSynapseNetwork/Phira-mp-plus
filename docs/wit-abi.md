...
| **MIGRATION_PHASE** | `2` (WIT-only) |
| **接口数量** | `12` |
| **Host traits** | `WitPluginHost` 已覆盖 12 个接口；配置、仿真、部分房间/用户/管理员接口已有基础实现，部分控制类和持久化查询函数仍返回明确错误 |
...
3. Host trait 骨架已实现，且 `phira-config`、`phira-simulation`、部分 `phira-room-mgmt` / `phira-user-mgmt` / `phira-admin` 函数已有基础实现。
4. `phira-plugin-sdk` 的 WIT exports、`call_on_event` 参数转换与 `on-api` 调用仍需继续完善。
5. 写能力 host API 的 capability enforcement 仍需补齐，不能把当前状态视为 ABI 完成。
6. 所有新插件应面向 `abi-wit-v2` 和 `phira-plugin-v2` world 编译。
...
但当前状态不能视为完成。下一步只做以下收敛工作:

1. 完成 WIT component `init` / `cleanup` / `on-event` / `on-api` 生命周期调用。
2. 将 `PluginEvent` 转换为 WIT typed event DTO，并为每个事件补合同测试。

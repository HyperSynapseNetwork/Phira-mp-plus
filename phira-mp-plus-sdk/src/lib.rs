...
//!
//! ⚠️ 此 SDK 已废弃（deprecated），仅作为开发参考保留。
//!
//! 请使用 `phira-mp-plus-server-api` crate 中的 `NativePlugin` trait 开发插件。
//! 原生插件示例见 `plugins/` 目录下的各个内置插件。
//!
//! # 迁移指南
//!
//! | 此 SDK | server-api 替代 |
//! |---|---|
//! | `PhiraPlugin` trait | `NativePlugin` trait |
//! | `PluginContext::log()` | `tracing::info!()` / `warn!()` |
//! | `PluginEvent` | `phira_mp_plus_server_api::PluginEvent` |
//! | `ServerApiRef` | `PluginContext` capabilities (`with_http`, `with_state`, `with_api`) |
...
/// 插件事件类型
///
/// ⚠️ 此枚举已废弃。请使用 `phira_mp_plus_server_api::PluginEvent`。
#[derive(Debug, Clone, Serialize, Deserialize)]
...
///
/// ⚠️ 此类型已废弃。请使用 `phira_mp_plus_server_api::PluginContext`。
#[derive(Debug, Clone)]
...
/// 插件特征
///
/// ⚠️ 此 trait 已废弃。请使用 `phira_mp_plus_server_api::NativePlugin`。
...
impl PhiraPlugin for ExampleLogger {
    ...
    fn on_connect(&self, ctx: &PluginContext, user_id: i32, user_name: &str) {
        ctx.log("info", &format!("[ExampleLogger] user {}({}) connected", user_name, user_id));
    }

    fn on_disconnect(&self, ctx: &PluginContext, user_id: i32, user_name: &str) {
        ctx.log("info", &format!("[ExampleLogger] user {}({}) disconnected", user_name, user_id));
    }

    fn on_room_create(&self, ctx: &PluginContext, user_id: i32, room_id: &str) {
        ctx.log("info", &format!("[ExampleLogger] user {} created room {}", user_id, room_id));
    }

    fn on_room_join(&self, ctx: &PluginContext, user_id: i32, room_id: &str, is_monitor: bool) {
        ctx.log("info", &format!(
            "[ExampleLogger] user {} joined room {} (monitor: {})",

...
impl CliHandler {
    ...
    /// CLI handler for `approve openuds <pending_id>`.
    /// Approves a pending OpenUDS connection in CLI approve mode.
    pub(in crate::cli) async fn dispatch_approve_command(&self, args: &[&str]) {
        let subcommand = args.first().copied().unwrap_or("");
        if subcommand != "openuds" {
            self.out(format!(
                "  {} {} approve openuds <pending_id>",
                c::yellow("?"),
                c::bold("用法")
            ));
            self.out("  ▸ 批准挂起的 OpenUDS 连接".to_string());
            return;
        }
        let pending_id = args.get(1).copied().unwrap_or("");
        if pending_id.is_empty() {
            self.out(format!(
                "  {} approve openuds <pending_id>",
                c::yellow("?")
            ));
            self.out("  ▸ 查看挂起的请求: 无需在 CLI 查看，OpenUDS 连接时会显示 pending_id".to_string());
            return;
        }
        ...
        match crate::openuds::get_auth_state() {
            Some(auth_state) => {
                if auth_state.approve_pending(pending_id).await {
                    self.out(format!("  {} 已批准 OpenUDS 连接: {}", c::green("✓"), pending_id));
                } else {
                    self.out(format!(
                        "  {} 未找到挂起的 OpenUDS 连接: {}",
                        c::red("✗"),

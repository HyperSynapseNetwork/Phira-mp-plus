    ...
    pub(in crate::cli) async fn runtime_cutover(&self, args: &[&str]) {
        if let Some(raw_mode) = args.get(1) {
            match crate::telemetry_batcher::TelemetryCutoverMode::parse(raw_mode) {
                ...
                None => {
                    self.out(format!(
                        "  {} unknown telemetry cutover mode: {}",
                        c::red("✗"),
                        c::yellow(raw_mode)
                    ));
                    let avail: Vec<&str> =
                        crate::telemetry_batcher::TelemetryCutoverMode::variants()
                            .iter()
                            .map(|m| m.as_str())
                            .collect();
                    self.out(format!("  available: {}", avail.join(" | ")));
                }
            }
        } else {
            ...
            self.out(format!(
                "  {} current: {}",
                c::dim("│"),
                c::bold(&stats.telemetry_cutover_mode)
            ));
            self.out(format!(
                "  {} changes: {}",
                c::dim("│"),
            ...
            for mode in crate::telemetry_batcher::TelemetryCutoverMode::variants() {
                let marker = if mode.as_str() == stats.telemetry_cutover_mode {
                    "*"
                } else {
                    " "
                };

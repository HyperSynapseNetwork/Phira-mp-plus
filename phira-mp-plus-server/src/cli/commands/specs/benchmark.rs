...
pub fn specs() -> Vec<CommandSpec> {
    ...
    out.push(
        ...
        .advanced()
        .arg(CommandArgSpec::optional("fixed|ramp", "运行模式：fixed 维持游玩房间上限；ramp 加压直到 CPU/RAM 触顶"))
        .arg(CommandArgSpec::optional("--playing-rooms", "fixed：最大同时在线游玩房间数（会话数自动 = 房间 × 2）"))
        ...
        .example("benchmark run fixed --playing-rooms 50 --duration 10m")
        .example("benchmark run fixed --playing-rooms 100 --forever")

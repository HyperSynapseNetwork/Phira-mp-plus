//! HTTP extension routes and the canonical Runtime SSE stream.
...
impl api::HttpHandleInner for HttpHandleBridge {
    ...
    fn register_sse(&self, path: &str, plugin: &str, event_types: &[String]) {
        ...
        if let Ok(mut streams) = self.0.sse_streams.try_write() {
            streams.insert(path.clone(), config);
            tracing::info!(path = %path, "SSE stream registered (plugin-backed)");
        } else {
            tracing::warn!(path = %path, "SSE stream registration deferred (lock contention)");
        }
    }
}

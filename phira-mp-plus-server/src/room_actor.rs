...
impl RoomCommandGateway {
    ...
    pub fn start_mailbox(self: &Arc<Self>, state: Arc<PlusServerState>, capacity: usize) {
        ...
        tokio::spawn(async move {
            while let Some(command) = rx.recv().await {
                ...
                gateway.execute_mailbox_command(&state, command).await;
            }
            gateway.mailbox_closed.fetch_add(1, Ordering::Relaxed);
        });
    }
    ...
    fn state_weak(&self) -> Option<Weak<PlusServerState>> {
        self.state_ref.read().ok().and_then(|guard| guard.clone())
    }

    fn self_arc(&self) -> Option<Arc<RoomCommandGateway>> {
        self.self_ref
            .read()
            .ok()
    ...
    fn room_mailbox_sender(&self, room_id: &str) -> Option<mpsc::Sender<RoomActorCommand>> {
        ...
        tokio::spawn(async move {
            while let Some(command) = rx.recv().await {
                ...
                let should_stop = matches!(command, RoomActorCommand::CloseRoom { .. });
                gateway.execute_mailbox_command(&state, command).await;
                if should_stop {
                    break;
                }
    ...
    async fn execute_mailbox_command(
        &self,
    ...
    async fn room_mailbox_or_inline<Build, Inline, Fut>(
        &self,
        room_id: &str,
        build: Build,
        inline: Inline,
    ...
    pub fn stats(&self) -> RoomCommandGatewayStats {
        ...
        RoomCommandGatewayStats {
            ...
            note: "set_lock/set_cycle/set_host/close now cross a per-room mailbox registry; kick/start/cancel still use the inline gateway facade".to_string(),
        }
    }

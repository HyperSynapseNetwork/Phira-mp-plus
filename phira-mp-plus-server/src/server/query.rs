...
fn server_state_query_dispatch(
    ...
    match method {
        ...
        "sse.register_stream" => {
            ...
            let Some(handle) = state.plugin_manager.http_handle() else {
                tracing::error!(%path, "sse.register_stream: http not enabled");
                return Err("http not enabled".to_string());
            };
            tracing::info!(%path, plugin = %plugin_name, "sse.register_stream: registering");
            handle.register_sse_stream(&path, &plugin_name, &event_types);
            tracing::info!(%path, "sse.register_stream: done");
            Ok(serde_json::json!({"ok": true, "path": path}))
        }
        "user.kick" => {
        ...
        "room.add_remote_player" => {
            ...
            let player_id = args
                ...
                .and_then(Value::as_i64)
                .map(|value| value as i32)
                .ok_or_else(|| "player_id required".to_string())?;
            ...
            let rid = room_id.to_string();
            spawn_on_runtime(async move {
                let _ = tx.send(s.add_remote_player(&rid, player_id, &player_name).await);
            });
            rx.recv_timeout(runtime_state_query_timeout())
                .unwrap_or(Err("room.add_remote_player timeout".to_string()))
        }
        "room.remote_ready" => {
            let room_id = args
                .first()
            ...
            spawn_on_runtime(async move {
                ...
                let _ = tx.send(
                    s.room_commands
                        .set_ready(&s, &rid, player_id, deadline, None)
                        .await,
                );
            });
            rx.recv_timeout(runtime_state_query_timeout())
                .unwrap_or(Err("room.remote_ready timeout".to_string()))
        }
        "room.remote_abort" => {
            let room_id = args
                .first()
            ...
            spawn_on_runtime(async move {
                let _ = tx.send(
                    s.room_commands
                        .abort_round(&s, &rid, player_id, None, None)
                        .await,
                );
            });
            rx.recv_timeout(runtime_state_query_timeout())
                .unwrap_or(Err("room.remote_abort timeout".to_string()))
        }
        "room.remote_leave" => {
            let room_id = args
                .first()
            ...
            let rid = room_id.to_string();
            spawn_on_runtime(async move {
                let _ = tx.send(s.remove_remote_player(&rid, player_id).await);
            });
            rx.recv_timeout(runtime_state_query_timeout())
                .unwrap_or(Err("room.remote_leave timeout".to_string()))
        }
        "admin.list" => {
            let (tx, rx) = std::sync::mpsc::channel();
            let s = Arc::clone(state);

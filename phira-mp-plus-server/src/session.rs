                        ...
                        if waiting_for_authenticate.load(Ordering::SeqCst) {
                            if let ClientCommand::Authenticate { token } = &cmd {
                                ...
                                if let Err(err) = res {
                                    ...
                                    {
                                        let welcome_user = Arc::clone(user);
                                        let welcome_server = Arc::clone(&server);
                                        let weak_self = this.get().map(Arc::downgrade);
                                        crate::supervisor_actor::spawn_named(
                                            format!("send-welcome-loop-{}", user.id),
                                            async move {
                                                ...
                                                loop {
                                                    ...
                                                    let in_room = welcome_user.room.read().await.is_some();

                                                    if in_room && !was_in_room {
                                                        // 刚进入房间：清屏 + 加入提示
                                                        let room_id = match welcome_user.room.read().await.as_ref() {
                                                            Some(room) => room.id.clone(),
                                                            None => { was_in_room = false; continue; }
                                                        };
                                                        if let Some(session) = weak_self.upgrade() {
                                                            for _ in 0..24 {
                                                                session.try_send(ServerCommand::Message(Message::Chat {
                                                                    user: 0,
                                                                    content: String::new(),
                                                                })).await;
                                                            }
                                                            session.try_send(ServerCommand::Message(Message::Chat {
                                                                user: 0,
                                                                content: format!("你已加入 {room_id}"),
                                                            })).await;
                                                        }
                                                        was_in_room = true;
                                                    } else if !in_room {
                                                        // 不在房间：周期性发送欢迎语并补空格到24行
                                                        let online = welcome_server.users.read().await.len();
                                                        let lines = crate::internal_hooks::send_welcome(
                                                            welcome_user.id,
                                                            &welcome_user.name,
                                                            online,
                                                            &welcome_server,
                                ...
                                let token_str = String::from_utf8_lossy(key);
                                match authenticate_remote_with_notice(
                                    &server,
                                    &token_str,
                                    PhiraRetryNoticeTarget::Stream(send_tx.as_ref()),
